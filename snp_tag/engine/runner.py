...
class CallbackLigero(Callback):
    ...
    def notify(self, algoritmo):
        ...
        F_gen = evaluar_poblacion_vectorizado(X_gen, prob.matriz_discrepancia)
...
def _ejecutar_replica_individual(args: Dict[str, Any]) -> ResultadoEjecucion:
    ...
    problema = ProblemaTagSNP(H, pair_idx, normalizar_busqueda=(nombre_algo == 'MOEAD'))
    ...
    if Xf.shape[0] > 0:
        Ff = evaluar_poblacion_vectorizado(Xf, problema.matriz_discrepancia)
    else:
...
def ejecutar_suite_completa(H, pair_idx, cfg: ConfiguracionExperimento):
    ...
    base_semillas = rng_semillas.integers(0, 10_000_000, size=cfg.n_ejecuciones)
    ...
    for algo in algoritmos:
        for init in cfg.opciones_init:
            for r in range(cfg.n_ejecuciones):
                semilla = int(base_semillas[r] + (hash((algo, init)) % 10000))
                trabajos.append({
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                print(f"      • ⚠ Error en [{t['algo']}-{t['init']}] ejecución {t['replica']}: {e}")
